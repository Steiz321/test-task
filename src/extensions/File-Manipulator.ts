...
export async function createPageFile(title: string, description: string, content: string, urlSlug: string) {
   return fs.writeFile(path.resolve('page/', `${urlSlug}.html` ), createHTMLTemplate(title, description, content), (err) => {
      if (err) throw err;
...
export async function removePageFile(urlSlug: string) {
   return fs.unlink(path.resolve('page/', `${urlSlug}.html` ), (err) => {
      if(err) throw err;
